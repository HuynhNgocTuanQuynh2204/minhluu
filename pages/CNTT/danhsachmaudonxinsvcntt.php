...
<main class="main container" id="main">
    <form class="form-inline" action="index.php?quanly=timkiemdondaguicntt" method="POST">
            ...
            <table>
                ...
                <tbody>
                    ...
                    while ($row = mysqli_fetch_array($query)) {
                        ...
                        $i++;
                        if ($row['donvicolienquan'] == 'Khoa Công nghệ thông tin') {
                    ?>
                            <tr>
                                <td><?php echo $i; ?></td>
                                ...
                                <td><?php echo ($row['ngayhoanthanh'] == 0) ? 'Chưa hoàn thành' : $row['ngayhoanthanh']; ?></td>
                                <td><?php echo $row['tinhtrang']; ?></td>
                                <?php if ($row['tinhtrang'] != 'Đã xác nhận') { ?>
                                    <td><a class="status pending" href="index.php?quanly=capnhaptrangthaichosinhviencntt&iddg=<?php echo $row['id_dg']; ?>">Cập nhập</a></td>
                                <?php } ?>
                                <?php if ($row['tinhtrang'] == 'Đã tiếp nhận đơn và gửi về đơn vị có liên quan') { ?>
                                    <td><a class="status cancelled" href="index.php?quanly=capnhapmaudon&iddg=<?php echo $row['id_dg']; ?>">Tiếp nhận đơn</a></td>
                                <?php } elseif ($row['tinhtrang'] != 'Đã hoàn thành đơn' && $row['tinhtrang'] != 'Đã xác nhận') { ?>
                                    <td><a class="status cancelled" href="index.php?quanly=xacnhanhoanthanh&iddg=<?php echo $row['id_dg']; ?>">Xác nhận hoàn thành</a></td>
                                <?php } ?>
                            </tr>
                    <?php
                        }
