...
<main class="main container" id="main">
    <form class="form-inline" action="index.php?quanly=timkiemformcntt" method="POST">
            ...
            <table>
                ...
                <tbody>
                    ...
                    while($row = mysqli_fetch_array($query)){
                        ...
                        $i++;
                        if($row['donvi'] == 'Khoa Công nghệ thông tin' ){
                    ?>
                    <tr>
                        <td><?php echo $i; ?></td>
                        ...
                        <td><?php if($row['thoigianhoanthanh']==0){
                         ...
                         <td><?php echo $row['tinhtrang']; ?></td>
                                <?php if ($row['tinhtrang'] != 'Đã xác nhận') { ?>
                                    <td><a class="status pending" href="index.php?quanly=caonhapformsvcntt&idform=<?php echo $row['id_form']; ?>">Cập nhập</a></td>
                                <?php } ?>
                                <?php if ($row['tinhtrang'] == 'Đã tiếp nhận đơn và gửi về đơn vị có liên quan') { ?>
                                    <td><a class="status cancelled" href="index.php?quanly=xacnhanformsv&idform=<?php echo $row['id_form']; ?>">Tiếp nhận đơn</a></td>
                                <?php } elseif ($row['tinhtrang'] != 'Đã hoàn thành đơn' && $row['tinhtrang'] != 'Đã xác nhận') { ?>
                                    <td><a class="status cancelled" href="index.php?quanly=xacnhanhoanthanhform&idform=<?php echo $row['id_form']; ?>">Xác nhận hoàn thành</a></td>
                                <?php } ?>
                            </tr>
                    <?php
                    }}
